...
describe('z.assets.AssetRemoteData', () => {
  describe('load unencrypted asset', () => {
    ...
    beforeEach(() => {
      ...
      spyOn(remote_data, '_loadBuffer').and.returnValue(Promise.resolve([video_bytes.buffer, video_type]));
    });
    ...
    beforeEach(done => {
      z.assets.AssetCrypto.encryptAesAsset(video_bytes)
        .then(({cipherText, keyBytes, sha256}) => {
          ...
          spyOn(remote_data, '_loadBuffer').and.returnValue(Promise.resolve([cipherText, video_type]));
          done();
        })
        .catch(done.fail);
    });
